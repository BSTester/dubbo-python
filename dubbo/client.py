...
class NcRegister(object):
    """
    NcRegister的主要作用：根据特定的interface从nacos中取出与之相关的所有provider的host并且监听provider的变化，当发生变化时更新最新的hosts到本地缓存；
    """

    def __init__(self, hosts, application_name='search_platform', username=None, password=None):
        """
        :param hosts: Nacos的地址
        :param application_name: 当前客户端的名称
        """
        nc = NacosClient(server_addresses=hosts, username=username, password=password)
        self.nc = nc
        self.hosts = {}
        self.weights = {}
        self.application_name = application_name
    ...
    def get_provider_host(self, interface):
        """
        从nacos中可以根据接口名称获取到此接口某个provider的host
        :param interface:
        :return:
        """
        if interface not in self.hosts:
            ...
            try:
                if interface not in self.hosts:
                    path = DUBBO_NC_PROVIDERS.format(interface)
                    if self.zk.exists(path):
                        self._get_providers_from_zk(path, interface)
                        self._get_configurators_from_zk(interface)
                    else:
